...
#       units=getStrFromSQL(dbConn,conn,"select Code from DICT025 where Category like '"+str(Category)+"' and Factor="+str(factor))
	#PD/FIS/MEV                         ^^^^^ for Category "FYDE"
	#PT/FIS/MEV instead of PD/FIS/MEV   vvvvv for Category "FYDE"
        units=getStrFromSQL(dbConn,conn,"select Code from DICT025 where Category like '"+str(Category)+"' and Factor="+str(factor)+" order by Code desc")
...
#---remove duplicates because of "inner join x4pro_x4cdat"
    ...
    for row in rows:
        DatasetID=row['DatasetID']; iPoint=row['iPoint']
        if DatasetID==prev_DatasetID and iPoint==prev_iPoint: continue
        prev_DatasetID=DatasetID; prev_iPoint=iPoint
        rows1.append(row)
    return rows1
