...
def getX4SqlSearchDALEG0(target,react):
    print('\n___getX4SqlSearchDALEG0: ['+target+'] ['+react+']')
    sql=str(""
	+"select *                                  \n"
	+" ,x1 as En,dx1 as dEn                     \n"
	+" ,y as Sig,dy as dSig                     \n"
...
#            print('@pt:'+str(ii)+'/'+str(lx)+') '+str(fullCode)+' '+str(DatasetID)+' '+str(YearRef1)+' '+Author1+" x:"+str(xx)+" y:"+str(yy)+" dy:"+str(dyy)+" dx:"+str(dxx))
        ...
        if DatasetID!=lastDatasetID:
            ...
            if DataCalc!='': lastDataset['Calc']=DataCalc
            x=[];     lastDataset['x']=x
            y=[];     lastDataset['y']=y
        ...
        dx.append(dxx)
        ii+=1
        print(' pt:'+str(ii)+'/'+str(lx)+') '+str(fullCode)+' '+str(DatasetID)+' '+str(YearRef1)+' '+Author1
	+" x:"+str(xx)+strRecalc+" y:"+str(yy)+" dy:"+str(dyy)+" dx:"+str(dxx))
    return datasets
