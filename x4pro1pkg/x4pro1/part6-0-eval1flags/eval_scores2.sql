-- select 'IAEA-CRP recommendations' as `Title`,'Evaluator-Flags' as `X4Pro-Tables`,datetime() as now;
select 'IAEA-CRP recommendations' as `Title`,'Evaluator-Flags' as `X4Pro-Tables`,datetime() as now
,max(UpdateDate) as `X4Pro-database` from X4UPDATE;

select tblfile.fileID
 ,substr(tblfile.srcFileDate,1,10) as srcFileDate
 ,tblflag.author as `compiler`
 ,tblflag.itype
...
where itype<>'expert_x4' and itype<>'stat'
