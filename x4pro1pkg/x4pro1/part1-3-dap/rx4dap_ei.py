...
def getX4SqlSearchDAP_Ei(target,react,x4sqlparam):
    print("___getX4SqlSearchDAP_Ei:",target,react)
    sql=str(""
	+"select *                           \n"
	+" ,round(An) as Anint               \n"
	+"from dap1                          \n"
...
def getDatasets4plot(rows,fx=1e-6,fy=1e3):
    ...
    for row in rows:
        ...
        nowDatasetSplit=DatasetID+' '+str(zz)+' '+str(elv)
        ...
        if nowDatasetSplit!=lastDatasetSplit:
            ...
            lastDataset['DatasetSplit']='ELv='+str(elv/1e6)+'MeV '+' An='+str(zz)+'deg'
...
#groups of datasets with the same Reacode
