...
SELECT x4.DatasetID,ds.year1 as YearRef1                              \n\
 ...
 ,x4.idat as iPoint,x4.xdat                                           \n\
 ,round(c5.y*c5.Fcm0,10) as ynew0sql                                  \n\
 ,c5.y,c5.dy                                                          \n\
 ,case                                                                \n\
...
--  and (Author1 like 'Zupranska' ) \n\
  and ((Author1 like 'Zupranska' )  \n\
      or (ds.year1>2000 and Author1 like 'Filatenkov')    \n\
      or (Author1 like 'Bostan')    \n\
--      or (Author1 like 'Bormann')   \n\
--      or (Author1 like 'Fessler')   \n\
      )    \n\
-- and (Author1 like 'Filatenkov'  or Author1 like 'Bostan' or Author1 like 'Zupranska' ) \n\
...
def getDatasets4plot(rows,fx=1e-6,fy=1e3):
    ...
    print('\n__getDatasets4plot: forming Datasets for plotting')
    print('datapoints:',len(rows))
    for row in rows:
        ...
        dx.append(dxx)
        ii+=1
        print('R:'+str(ii)+'/'+str(lx)+') '+str(fullCode)+' '+str(DatasetID)+' '+str(YearRef1)+' '+Author1+" x:"+str(xx)+" y:"+str(yy)+" dy:"+str(dyy)+" dx:"+str(dxx)
		+" Fcm0:"+str(row['Fcm0'])
		+" dm0:"+str(row['dm0'])
		+" dm1:"+str(row['dm1'])
		#+" FcDecayData:"+str(row['FcDecayData'])
		#+" FcDecayMon:"+str(row['FcDecayMon'])
		)
    return datasets


#Turn list of sql.Row objects into list of dictionaries
def rows2dicts(rows):
    newrows=[]
    for row in rows:
        newrow=dict(row) #newrow=dict(zip(row.keys(),row))
        xdat=newrow.get('xdat')
        if xdat is not None: newrow['xdat']=json.loads(xdat)
...
def getDatasets00(rows):
    ...
    print('\n__getDatasets00: form Datasets consist of retieved data points')
    print('datapoints:',len(rows))
    for row in rows:
        ...
        if DatasetID!=lastDatasetID:
            ...
            lastDatasetID=DatasetID
            print(str(len(datasets))+') '+'___________#'+DatasetID)
        lastRows.append(row);
        ii+=1
